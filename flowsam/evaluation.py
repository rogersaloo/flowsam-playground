...
def run_flowpsam(args, flowsam, info):
    with torch.no_grad():
        ...
        input_size = (
            int(original_size[0] * 1024 / max(*original_size)),
            int(original_size[1] * 1024 / max(*original_size)),
        )
        ...
        for coords_idx in range(grid_coords_set.shape[0] // 10):
            ...
            point_labels = torch.ones(
                grid_coords.size()[:2], dtype=torch.int, device=grid_coords.device
            )
            point_prompts = (grid_coords, point_labels)
            masks_logit, fiou, mos = flowsam(
                rgb_image, flow_image, point_prompts, use_cache=True
            )
            fiou = fiou[:, args.sam_channel]
            mos = mos[:, 0]
            score = fiou + mos
            masks_logit = masks_logit[..., : input_size[0], : input_size[1]]
            masks_logit = F.interpolate(
                masks_logit, original_size, mode="bilinear", align_corners=False
            )
...
def eval(args, val_loader, flowsam):
    ...
    for idx, info in enumerate(val_loader):
        ...
        if os.path.dirname(info["path"][0]) not in iou_summary.keys() and (
            "num_obj" in info.keys()
        ):
        ...
        keep_idx = batched_nms(
            boxes_set, scores_set, torch.zeros_like(boxes_set[:, 0]), iou_threshold=0.9
        )
        masks_fil, scores_fil, boxes_fil = filter_data(
            [masks_set, scores_set, boxes_set], keep_idx, is_idx=True
        )

        # Removing bg masks
        keep_maskidx = ~is_bg_mask(masks_fil)
        masks_fil, scores_fil, boxes_fil = filter_data(
            [masks_fil, scores_fil, boxes_fil], keep_maskidx
        )
        ...
        # Overlaying masks
        masks_nonhung, saved_mask_nonhung = hard_thres(
            masks_nonhung, scores, output_savemask=True
        )
        # Padding masks to match with num_obj
        if masks_nonhung.shape[0] < max(args.max_obj, anno.shape[1]):
            masks_nonhung_pad = torch.repeat_interleave(
                torch.zeros_like(masks_nonhung[0:1], device=masks_nonhung.device),
                max(args.max_obj, anno.shape[1]) - masks_nonhung.shape[0],
                0,
            )
        ...
        if "anno" in info.keys():
            ...
            orig_idx, hung_idx = linear_sum_assignment(
                -result_iou.cpu().detach().numpy()
            )
            masks_hung = masks_nonhung[hung_idx]  # Hungarian matched masks
            iou_summary = update_iousummary(
                masks_hung,
                masks_nonhung,
                anno,
                info["num_obj"].item(),
            ...
            if args.save_path:
                ...
                os.makedirs(
                    os.path.dirname(os.path.join(save_path_nonhung, info["path"][0])),
                    exist_ok=True,
                )
                save_indexed(
                    os.path.join(save_path_nonhung, info["path"][0]),
                    saved_mask_nonhung.astype(np.uint8),
                )
