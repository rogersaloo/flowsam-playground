...
class Example_eval_dataset(Dataset):
    def __init__(
        ...
        self.point_grids = build_all_layer_point_grids(
            n_per_side = num_gridside, n_layers = 0, scale_per_layer = 1)[0]
        ...
        for idx, seq in enumerate(sorted(self.seqs)):
            ...
            for filename in sorted(os.listdir(seq_dir)):
                ...
                flow_path = os.path.join(
                    seq_dir.replace(self.rgb_dir, self.flow_dir), filename.replace(".jpg", ".png"))
                rgb_path = os.path.join(
                    seq_dir.replace(self.rgb_dir, self.rgb_dir), filename.replace(".png", ".jpg"))
    ...
    def val_loading(self, idx):
        ...
        for flow_gap in self.flow_gaps:
            ...
            if os.path.exists(flow_path):
                ...
                flow_path = self.flow_paths[idx].replace(
                    "FlowImages_gap1", "FlowImages_gap{}".format(-1 * flow_gap))
                flow_image = readRGB(flow_path, gt_resolution = original_size)
